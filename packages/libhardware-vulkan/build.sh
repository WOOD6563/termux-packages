...
termux_step_make() {
	# /build
	if ! $TERMUX_ON_DEVICE_BUILD; then
		echo "Trying to link with libvndksupport.so off-device will fail."
		echo "Please build on-device and upload the package."
	fi
	...
	clang \
		*libhardware/hardware.c \
		-g \
		-I *libhardware/include \
		-I core/include \
		-I core/libvndksupport/include \
		-I logging/liblog/include \
		-shared \
		-lcutils \
		-lvndksupport \
		-llog \
		-o "$TERMUX_PKG_BUILDDIR/libhardware.so"
}
